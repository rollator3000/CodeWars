...
FROM house_tasks 
LEFT JOIN task_status using (task_id) # Merge both DF based on 'task_id'
GROUP BY house_id
ORDER BY house_id DESC

# 6-11  SQL Sleuths: Exposing the Developer's Attributes
# Your task is to write and execute a SQL query that illustrates the various stages in a developer's career, along with their associated attributes.
# (1) developer_attributes table with the following columns:
...
# In your result set, use the column name description to display the data. 
# Developer titles should be distinctively formatted to stand out. Enclose them in whitespaces and asterisks like *** Trainee ***.

# The output should be a list where each developer title is followed by its respective attributes, clearly differentiating between the titles and their attributes.
...
# 1) Create a titles-table with developer_title in *** ***, + the orginal developer title
WITH titles AS (
    SELECT DISTINCT 
        '*** ' || developer_title || ' ***' AS description, 
        developer_title
    FROM developer_attributes
),

# 2) Create a attributes-table with all attributes per developer_title
attributes AS (
    SELECT 
...
# 3) Merge descriptions of 'titles' and 'attributes' and order them by developer_title & description
SELECT description
FROM (
    SELECT * FROM titles
    UNION ALL
...
ORDER BY developer_title, description ASC
